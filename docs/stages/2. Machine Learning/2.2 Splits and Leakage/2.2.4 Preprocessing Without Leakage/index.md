# 2.2.4 Preprocessing Without Leakage

## Why This Sub-Part Matters

Preprocessing Without Leakage is the working skill inside Splits and Leakage that helps you build the stage artifact, An ML baseline report comparing simple and stronger models with metrics, error slices, and a model card, while collecting enough evidence to trust the result. A sub-part is now a folder so longer topics can grow without forcing everything into one huge page.
...
- Define Preprocessing Without Leakage in plain language before naming tools or frameworks.
...
- Measure it with: split sizes, group boundaries, time boundaries, duplicates, and leakage risks
...
4. Add one measurement using: Track split sizes, group boundaries, time boundaries, duplicates, and leakage risks.

## Completion Standard

- I can explain Preprocessing Without Leakage without naming a tool first.
...
Return to [2.2 Splits and Leakage](<../index.md>).

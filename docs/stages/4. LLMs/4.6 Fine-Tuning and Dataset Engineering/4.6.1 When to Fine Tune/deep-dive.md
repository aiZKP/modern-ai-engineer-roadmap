...
- Input: identify what raw information, code, data, prompt, model output, trace, or user signal When to Fine Tune consumes.
...
- Failure mode: record how When to Fine Tune can fail specifically in LLMs, not only in theory.
...
A strong learner can point to a small artifact, explain the tradeoff, show a measurement, and name the next improvement. For When to Fine Tune, that means the explanation is grounded in Fine-Tuning and Dataset Engineering and the stage artifact rather than floating as general AI vocabulary.

Return to [4.6.1 When to Fine Tune](<index.md>).

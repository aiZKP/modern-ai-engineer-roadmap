# Deep Dive: Tool Contracts and JSON Schema
...
- Input: identify what raw information, code, data, prompt, model output, trace, or user signal Tool Contracts and JSON Schema consumes.
...
- Failure mode: record how Tool Contracts and JSON Schema can fail specifically in AI Agents, not only in theory.
...
A strong learner can point to a small artifact, explain the tradeoff, show a measurement, and name the next improvement. For Tool Contracts and JSON Schema, that means the explanation is grounded in Tool Design and the stage artifact rather than floating as general AI vocabulary.

Return to [6.3.1 Tool Contracts and JSON Schema](<index.md>).

...
Function Calling is the working skill inside Tool Design that helps you build the stage artifact, A tool-using agent with typed tools, memory, traces, task evals, prompt-injection tests, and an architecture README, while collecting enough evidence to trust the result. Treat it as a small engineering contract: what enters, what changes, what leaves, how you know it worked, and how it can fail.

## Key Mechanisms

- Input: identify what raw information, code, data, prompt, model output, trace, or user signal Function Calling consumes.
...
- Failure mode: record how Function Calling can fail specifically in AI Agents, not only in theory.
...
A strong learner can point to a small artifact, explain the tradeoff, show a measurement, and name the next improvement. For Function Calling, that means the explanation is grounded in Tool Design and the stage artifact rather than floating as general AI vocabulary.

Return to [6.3.2 Function Calling](<index.md>).
